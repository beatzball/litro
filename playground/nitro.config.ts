...
export default defineNitroConfig({
  ...
  handlers: [
    ...
    },
    {
      ...
      // replays/live-tails it). Same timing trap as actions above: the
    ...
    {
      // MCP Streamable HTTP. Two entries for one route: POST is the whole
      // JSON-RPC channel, and OPTIONS is the CORS preflight a browser-based
      // inspector sends before it — which must answer without the bearer
      // token, because a browser will not attach one to a preflight. GET is
      // deliberately absent: protocol revision 2026-07-28 removes the
      // standalone stream, and this route is stateless.
      //
      // Same timing trap as the routes above (NITRO-001): the handler stub is
      // generated by the litro agents plugin in build:before, but the route
      // itself must be declared here because the dev server reads handler
      // config before that hook fires.
      route: '/__litro/mcp/:agent',
      method: 'post',
      handler: resolve('./server/stubs/mcp-handler.ts'),
    },
    {
      route: '/__litro/mcp/:agent',
      method: 'options',
