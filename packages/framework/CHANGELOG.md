...
- 4cdba19: Server-render `<litro-link>`, and put the skip links inside a landmark.
...
- Updated dependencies [818ba3f]
  - @beatzball/litro-router@0.3.1

## 0.17.0

### Minor Changes

- bfe941d: Add `litro mcp serve`: serves one agent's tools, and the `ui://` documents
...
- 1f347da: Export `normalizePathname`, the one canonicalization every route matcher shares.
...
- Updated dependencies [1f347da]
  - @beatzball/litro-router@0.3.0

## 0.16.3

### Patch Changes

- 8b5172c: Give every page a correct box model inside its shadow root.
...
- 1348cfa: Keep a page's element registrations in the Nitro server bundle.
...
- df9971f: Add `litro mcp-app build` — packs every `mcp-apps/*.ts` into a self-contained MCP Apps `ui://` document.
...
- 15c112d: Add `litro docs sync` and `litro docs check` for starlight docs sites.
...
- 83c8d5a: Remove the `source` export condition. It pointed consumers at the package's
...
- 7602471: Server Actions milestone 2: progressive-enhancement forms and streaming returns.
...
- a7d2dc1: Fix: emitted `.d.ts` files for the Elena adapter no longer reference a nonexistent `ElenaClass` type from `@elenajs/core`. Downstream consumers with strict TypeScript can now extend `LitroPage` without TS4113/TS2339 errors.
...
- 65d309a: fix(content): add rehype-raw to content parser pipeline
...
- f768829: fix(shell): escape `</script` in serverDataJson to prevent early script tag termination
