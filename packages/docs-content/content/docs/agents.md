...
3. **Elena `UIResult` renderer** (light-DOM, no hydration).
4. **Subagents, additional surfaces, scheduled runs, and eval suites.**
5. **`create-litro` template wiring** — until then, wire agents by hand per [Setup](#setup).
