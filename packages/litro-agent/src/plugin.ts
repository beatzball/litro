 ...
 *      feeds the manifest + config into createAgentHandler() — and
 *      `server/stubs/mcp-handler.ts`, which feeds the same manifest into
 *      createMcpHandler() for the MCP Streamable HTTP route. Both stubs are
 *      written whether or not the project declares their routes: an unused
 *      stub is never pulled into rollup's module graph, and generating it
 *      unconditionally is what lets a project turn the route on by adding two
 *      lines to nitro.config.ts and nothing else.
...
const MCP_HANDLER_STUB_REL = join('server', 'stubs', 'mcp-handler.ts');
...
/**
 * The MCP Streamable HTTP route.
 *
 * SHAPED EXACTLY LIKE `HANDLER_STUB_SOURCE` below, and that is deliberate: one
 * convention for generated handlers, so a reader who has seen one knows where
 * the next one's manifest comes from. Three differences, each forced:
 *
 *   - It imports `@beatzball/litro-agent/mcp-handler`, not `/mcp-server`. The
 *     `/mcp-server` subpath is the stdio entry point and reaches `fast-glob`
 *     through the project scanner; a server bundle has no use for a filesystem
 *     globber.
 *   - It takes no agent config. The MCP route needs the manifest and the
 *     agent's own `access` guard, and neither a provider nor a session store.
 *   - It is safe to generate in a project with no MCP SDK installed, because
 *     nothing on that import path loads the SDK until a request arrives.
 */
const MCP_HANDLER_STUB_SOURCE = `// @ts-nocheck
// @generated by litro agent scanner — do not edit
// Runtime handler for /__litro/mcp/:agent (MCP Streamable HTTP) — generated by
// the litro agents plugin; the route is declared in nitro.config.ts handlers.
import { createMcpHandler } from '@beatzball/litro-agent/mcp-handler';
import { agentEntries } from '#litro/agent-manifest';

export default createMcpHandler(agentEntries);
`;
...
export default async function agentsPlugin(nitro: Nitro): Promise<void> {
  async function runScan(): Promise<void> {
    ...
    await writeStub(rootDir, MCP_HANDLER_STUB_REL, MCP_HANDLER_STUB_SOURCE);
  ...
  //     // MCP Streamable HTTP, two entries: POST is the JSON-RPC channel and
  //     // OPTIONS is the CORS preflight a browser-based host sends first.
