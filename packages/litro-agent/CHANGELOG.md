...
- 49c281a: Serve an agent's tools and `ui://` apps over MCP Streamable HTTP, as a Nitro route.
...
- bfe941d: Add an MCP server behind a new `./mcp-server` subpath: one agent's tools, and
...
- bf8b54e: `runtime` and `apply` are parsed at build time, and a replaced fill step says so
...
- 5c09b18: MCP Apps: `uri` is now optional in `defineMcpApp()`
