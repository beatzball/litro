...
import { fillInnerHTMLBindings } from './postbuild-fill-innerhtml.js';
...
export default defineNitroConfig({
  ...
  // Keep @microsoft/fast-* external to avoid dual-copy SSR issues.
  // Only inline local page components so esbuild compiles their decorators.
  ...
  hooks: {
    ...
    'compiled': async () => {
      if (mode === 'static') {
        await fillInnerHTMLBindings(resolve('./dist/static'));
      }
    },
  },
