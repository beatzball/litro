...
export function renderStarlightConfig(repo: RepoInfo, siteRelPath: string): string {
  ...
  // '/docs' and not '/docs/getting-started': this is the only link to exactly
  // /docs in the site, and the recipe names no explicit prerender routes, so
  // the crawler seeds from '/' and follows links. Point it one level deeper and
  // /docs never prerenders — a 404 on a static host, a 403 on one that refuses
  // to list a directory with no index.html.
  nav: [
    { label: 'Docs', href: '/docs' },${githubNav}
  ],
