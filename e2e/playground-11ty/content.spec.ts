...
test('home page renders site title', async ({ page }) => {
  ...
  await expect(page.locator('page-home:not([hidden]) h1')).toContainText('playground-11ty');
});

test('home page lists recent posts', async ({ page }) => {
  await page.goto('/');
  await page.waitForSelector('page-home:not([hidden])');
  const items = page.locator('page-home:not([hidden]) ul li');
...
test('blog index renders', async ({ page }) => {
  ...
  await expect(page.locator('page-blog:not([hidden]) h1')).toContainText('Blog');
});

test('blog post renders title', async ({ page }) => {
  await page.goto('/blog/hello-world');
  await page.waitForSelector('page-blog-slug:not([hidden])');
  await expect(page.locator('page-blog-slug:not([hidden]) h1')).toBeVisible();
});
...
test('tags page renders filtered posts', async ({ page }) => {
  ...
  await page.waitForSelector('page-tags-tag:not([hidden])');
  // Should show at least one post link
  const links = page.locator('page-tags-tag:not([hidden]) a[href*="/blog/"]');
