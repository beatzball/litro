...
**Lighthouse under-reports DSD pages.** Lighthouse's scoring doesn't fully account for Declarative Shadow DOM — it can miss content inside `<template shadowrootmode="open">` for certain metrics, so Lit/FAST SSR pages score lower than the rendered output actually warrants. If Lighthouse scores are a hard requirement for you, Elena (light DOM) and SSG builds avoid this; Lit/FAST SSR pages need you to look past the score.
