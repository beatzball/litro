 ...
 *   Inline color-coded chip. Use `text` attribute for label.
 * </litro-badge>
 */
export class LitroBadge extends Elena(HTMLElement) {
  static tagName = 'litro-badge';
  static props = ['variant', 'text'];

  variant: BadgeVariant = 'default';
  text = '';

  render() {
    const v = (['note', 'tip', 'caution', 'danger'] as string[]).includes(this.variant) ? this.variant : 'default';
    return html`
      <style>
        @scope (litro-badge) {
          :scope {
      ...
      <span class="badge ${v}">${this.text}</span>
    `;
  }
}
