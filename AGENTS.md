...
Traps an agent has already hit in this repo. Each rule has a stable ID, a reason
and a check. Read the file before you work in its area.

- `.agents/rules/adapters-ssr.md` — before you touch a Lit, FAST or Elena component, an adapter, or a recipe's adapter template.
- `.agents/rules/build-bundling.md` — before you change a `package.json` exports map, a tsconfig, a Vite or Rollup config, or a module that runs on both server and browser.
- `.agents/rules/nitro-server.md` — before you touch Server Actions, generated server files, page data serialization, or the `litro dev` server.
- `.agents/rules/content-docs-site.md` — before you edit content, the content pipeline, `docs/`, `docs-ssr/`, `packages/docs-ui/`, or a recipe stylesheet.
- `.agents/rules/testing.md` — before you write or debug an e2e spec, a test fixture, a scaffolding check, or a benchmark run.
